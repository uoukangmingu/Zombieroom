...
function updateServerPlayers(room, dt) {
  ...
  for (const player of room.players.values()) {
    ...
    else if (!wantsSprint && m.grounded) m.stamina = Math.min(ps.maxStamina, m.stamina + SERVER_PLAYER.staminaRegen * (rawMove > .01 ? 1 : 1.12) * dt);
...
httpServer.listen(PORT, () => console.log(`BOXHEAD: BACKROOM LOW v62.0 server running at http://localhost:${PORT}`));
